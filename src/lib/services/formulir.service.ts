...
export class FormulirService {
   ...
   */
  static async saveAllFormulirData(
    userId: string,
    data: Partial<FormulirCompleteData>,
  ): Promise<FormulirServiceResponse<FormulirCompleteData>> {
    try {
      const results: Partial<FormulirCompleteData> = {};

      // Save each step if data is provided
      if (data.sekolah) {
        const sekolahResult = await this.saveSekolahInfo(userId, {
          nama_sekolah: data.sekolah.nama_sekolah,
          npsn: data.sekolah.npsn,
          nama_kepala_sekolah: data.sekolah.nama_kepala_sekolah || '',
          nip_kepala_sekolah: data.sekolah.nip_kepala_sekolah || '',
          alamat_sekolah: data.sekolah.alamat_sekolah || '',
          kecamatan: data.sekolah.kecamatan || '',
          phone: data.sekolah.phone || undefined,
        });
        if (!sekolahResult.success) {
          return { success: false, error: sekolahResult.error };
        }
        results.sekolah = sekolahResult.data;
      }

      if (data.guru) {
        const guruResult = await this.saveGuruData(userId, {
          dataGuru: data.guru,
        });
        if (!guruResult.success) {
          return { success: false, error: guruResult.error };
        }
        results.guru = guruResult.data;
      }

      if (data.rombonganBelajar) {
        const rombonganResult = await this.saveRombonganBelajarData(userId, {
          dataRombonganBelajar: data.rombonganBelajar,
        });
        if (!rombonganResult.success) {
          return { success: false, error: rombonganResult.error };
        }
        results.rombonganBelajar = rombonganResult.data;
      }

      if (data.sarana) {
        const saranaResult = await this.saveSaranaData(userId, {
          dataSarana: data.sarana,
        });
        if (!saranaResult.success) {
          return { success: false, error: saranaResult.error };
        }
        results.sarana = saranaResult.data;
      }

      if (data.prasarana) {
        const prasaranaResult = await this.savePrasaranaData(userId, {
          dataPrasarana: data.prasarana,
        });
        if (!prasaranaResult.success) {
          return { success: false, error: prasaranaResult.error };
        }
        results.prasarana = prasaranaResult.data;
      }

      if (data.kebutuhanPrioritas) {
        const kebutuhanResult = await this.saveKebutuhanPrioritasData(userId, {
          dataKebutuhanPrioritas: data.kebutuhanPrioritas,
        });
        if (!kebutuhanResult.success) {
          return { success: false, error: kebutuhanResult.error };
        }
        results.kebutuhanPrioritas = kebutuhanResult.data;
      }

      if (data.lampiran) {
        const lampiranResult = await this.saveLampiranData(userId, {
          dataLampiran: data.lampiran,
        });
        if (!lampiranResult.success) {
          return { success: false, error: lampiranResult.error };
        }
        results.lampiran = lampiranResult.data;
      }

      // Get complete updated data
      const completeDataResult = await this.getCompleteFormulirData(userId);
      if (!completeDataResult.success || !completeDataResult.data) {
        return { success: false, error: completeDataResult.error };
      }

      return {
        success: true,
        data: completeDataResult.data,
      };
    } catch (error) {
